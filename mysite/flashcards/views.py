...
from .models import Flashcard,Set, Card
...
def flashcards_list(request):
    return render(request, 'flashcards/decks.html', { })

def flashcard_detail(request,card_id ):
    card =  get_object_or_404(Poll,id= card_id )
    context = {'card': card}
    return render(request, 'flashcards/card_detail.html', context)
...
def decks(request):
    ...
    return render(request, 'flashcards/decks.html', {'all_sets' : all_sets, 'set_lens': set_lens, 'admin': admin})
    # return render(request, 'flashcards/decks.html')



# Flashcard set page
def review(request, set_id):
